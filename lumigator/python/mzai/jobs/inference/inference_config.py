...
SerializableTorchDtype = Annotated[
    torch.dtype | str, BeforeValidator(lambda x: _validate_torch_dtype(x))
]
...
class PipelineConfig(BaseModel, arbitrary_types_allowed=True):
    model_path: AssetPath = Field(title="The Model HF Hub repo ID", exclude=True)
    task: SupportedTask
    src_lang: str | None = None  # Source language for translation tasks
    tgt_lang: str | None = None  # Target language for translation tasks
    ...
    torch_dtype: SerializableTorchDtype = "auto"
...
class JobConfig(BaseModel, arbitrary_types_allowed=True):
