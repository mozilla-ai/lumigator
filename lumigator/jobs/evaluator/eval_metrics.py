...
class EvaluationMetrics:
    def __init__(self, metrics):
        ...
        self._supported_metrics = {
            ...
            "g_eval_translation": {
                "method": functools.partial(self._g_eval, task="translation"),
                "requires": [EvaluationFields.GROUND_TRUTH, EvaluationFields.EXAMPLE],
            },
            "g_eval_translation_noref": {
                "method": functools.partial(self._g_eval, task="translation", use_ref=False),
                "requires": [EvaluationFields.EXAMPLE],
            },
            "token_length": {
    ...
    def _g_eval(self, examples: list, pred: list, ref: list, task: str, use_ref: bool = True) -> dict:
        ...
        Args:
            examples: a list of original inputs to the evaluated LLM (e.g. texts to summarize)
            pred: a list of predictions from the evaluated LLM (e.g. summaries generated by the LLM)
            ref: a list of reference outputs to compare the predictions to (e.g. human-generated summaries)
            task: the name of the task (e.g. summarization, translation) as specified in `g_eval_prompts.json`
            use_ref: a boolean value specifying whether predictions will be evaluated wrt the reference outputs or not

        Returns:
            dict: Dictionary with keys matching different metrics and values as float numbers

        Raises:
            KeyError: If a wrong key (task name) is provided to access a task-specific prompt
        ...
        # add GT if working in supervised setting
        if use_ref:
