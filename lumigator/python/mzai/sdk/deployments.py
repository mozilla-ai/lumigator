...
class Deployments:
    ...
    def create_deployment(self, deployment: GroundTruthDeploymentCreate) -> GroundTruthDeploymentResponse:
        """Creates a new groundtruth deployment."""
        response = self.client.get_response(f'{self.EXPERIMENTS_ROUTE}'), HTTPMethod.POST, dumps(deployment)
...
#         return GroundTruthDeploymentResponse(**data)
    ...
    def delete_deployment(self, deployment_id: UUID) -> GroundTruthDeploymentResponse:
        """Returns information on the deployment for the specified ID."""
        UUID(deployment_id)
        response = self.client.get_response(f'{self.EXPERIMENTS_ROUTE}/{deployment_id}', HTTPMethod.DELETE)
    ...
    def get_deployments(self, skip: int = 0, limit: int = 100) -> ListingResponse[GroundTruthDeploymentResponse]:
