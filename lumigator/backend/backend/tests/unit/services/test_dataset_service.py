...
def test_delete_dataset_file_not_found(db_session, fake_s3fs):
    ...
    dataset_service = DatasetService(dataset_repo, fake_s3fs)
...
def test_upload_dataset(db_session, fake_s3fs, valid_upload_file):
    dataset_service = DatasetService(DatasetRepository(db_session), fake_s3fs)
...
def test_dataset_download_with_extensions(db_session, fake_s3fs, valid_upload_file, extension, total):
    dataset_service = DatasetService(DatasetRepository(db_session), fake_s3fs)
    upload_response = dataset_service.upload_dataset(valid_upload_file, DatasetFormat.JOB)
    ...
    download_response = asyncio.run(dataset_service.get_dataset_download(upload_response.id, extension))
